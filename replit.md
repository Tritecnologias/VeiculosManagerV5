...
- March 21, 2026. Firebase FCM push notifications: installed firebase-admin and @capacitor/push-notifications; added device_tokens table (userId, token, platform); created server/services/fcmService.ts (Firebase Admin SDK init, sendPushNotification, getAdminTokens); added POST/DELETE /api/fcm-tokens endpoints; auto-trigger FCM notification to admin users on vehicle creation; created client/src/hooks/use-push-notifications.ts (Capacitor native only, requests permissions, sends token to backend); integrated hook in App.tsx; updated .env.example with FIREBASE_SERVICE_ACCOUNT_JSON documentation
```
